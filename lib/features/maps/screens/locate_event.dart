...
class _CitySearchMapScreenState extends State<CitySearchMapScreen> {
  ...
  Future<void> _saveLocation(LatLng location) async {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Emplacement enregistré: ${location.latitude}, ${location.longitude}')),
    );
...
Future<Map<String, dynamic>?> getCoordinatesFromPlaceName(String placeName) async {
  final String apiKey = '5b3ce3597851110001cf6248465d6b0ae5b34c62881034d3a7aada1b'; // Replace with your OpenRouteService API key
  final String url = 'https://api.openrouteservice.org/geocode/search?text=$placeName&api_key=$apiKey';
